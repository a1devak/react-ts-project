...
export function DateForm({ value, onChange }: any) {
  return (
...
export function DateTimeForm({ filter, operator, value, onChange }: any) {
  ...
  return (
    <div className="date-time-form">
      ...
      <FormField description="Time" constraintText="Use 24-hour format.">
        <TimeInput format="hh:mm:ss" placeholder="hh:mm:ss" value={timeValue} onChange={(event) => onChangeTime(event.detail.value)} />
      </FormField>
...
function parseDateTimeFilter(filter: any) {
...
function isValidIsoDate(isoDate: any) {
  return !isNaN(new Date(isoDate).getTime());
}

function parseValue(value: any, defaultTime = "") {
  const [datePart = "", timePart = ""] = (value ?? "").split("T");
  return { dateValue: datePart, timeValue: timePart || defaultTime };
}

function formatTimezoneOffset(isoDate: any, offsetInMinutes?: any) {
